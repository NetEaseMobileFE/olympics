...
export default class extends Component {
    ...
    setFilter = index => {
    	this.props.setFilter(index);
	};

    render() {
        let { filter, medals, organisationImgUrl, organisationName, list, noMore, size } = this.props;
				...
				<div styleName="info">
					<div styleName="org-title"><img src={organisationImgUrl}/>{organisationName}</div>
					{
						medals && medals.length ?
							<div styleName="total-medals">累计获得
								<span onClick={this.setFilter.bind(this, 0)}>{medals[0]}</span>金
								<span onClick={this.setFilter.bind(this, 1)}>{medals[1]}</span>银
								<span onClick={this.setFilter.bind(this, 2)}>{medals[2]}</span>铜</div>
						 	: null
					}
				</div>
                {
					list.slice(0, size).filter(day => filter !== null ? day.medals[filter] > 0 : true).map((v, i) => {
                        ...
                        return (
                            <section styleName="list" key={i}>
                                ...
                                <table styleName="table-medal">
                                    ...
                                    <tbody>
                                        {v.competitions.filter(cmp => filter !== null ? cmp.medalType == medalTypes[filter] : true).map((v, i) => {
                                            return (
                                                <tr key={i}>
